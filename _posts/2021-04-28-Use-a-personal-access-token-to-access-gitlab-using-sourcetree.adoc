---
layout: post
title:  "Use a personal access token to set up a GitLab.com account profile in SourceTree"
categories: [Technology]
tags: [git, gitlab, sourcetree, bitbucket, dvcs, token, personal, access]
...
IMPORTANT: If your GitLab.com account uses Two-factor Authentication (2FA) you *must* use a personal access token as your password to connect over HTTPS.
...
== Use the personal access token to authenticate to GitLab through SourceTree Desktop
...
You can now clone repositories from the GitLab.com remote host.
...
. Check the *Clone the repository* modal displays the `https://` prefix in the Source URL.
