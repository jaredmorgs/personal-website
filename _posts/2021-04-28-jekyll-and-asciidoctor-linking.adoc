---
layout: post
title: "Jekyll blogs do not support xref Asciidoctor syntax"
categories: [Technology]
tags: [jekyll, asciidoctor, link, xref]
image: assets/images/<<<something here>>>
featured: true
#hidden: true
#rating: 4.5
---
...
I found out that when you need to link between posts in a Jekyll site, you need to use the https://docs.asciidoctor.org/asciidoc/latest/macros/link-macro/[`link` macro] and pay special attention to the `permalink` attribute in your Jekyll template's `_config.yml` file.

== Background

When setting up this site, I wanted to link two GitLab related posts together because the compliment each other:

* link:use-a-token-to-access-gitlab-using-sourcetree[Use a personal access token to access GitLab using SourceTree]
